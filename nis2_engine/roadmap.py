...
# Fases de remediação por prioridade — alinhadas com PRIORITY_BY_FUNCTION em
...
PHASE_ORDER = {"alta": 0, "media": 1, "baixa": 2}
# Aliases retrocompatíveis (nomes privados anteriores).
_PHASES_BY_PRIORITY = PHASES_BY_PRIORITY
...
def build_remediation_roadmap(result: AssessmentResult) -> RemediationRoadmap:
    ...
    phases: list[RoadmapPhase] = []
    for priority in sorted(PHASES_BY_PRIORITY, key=lambda p: PHASE_ORDER[p]):
        name, timeframe = PHASES_BY_PRIORITY[priority]
